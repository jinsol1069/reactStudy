    ...
    const loadShoesByOnce = () => {
        ...
        axios1
            .then((result1) => {
                ...
                updateShoes([...shoes, ...result1.data]);
                return axios2;
            })
            .then((result2) => {
                // axios2 의 결과값이 전달 됨
                console.log(result2.data);
                updateShoes([...shoes, ...result2.data]);
            })
            .finally(() => {
                setLoading(false);
            });
        // Promise.all([axios1, axios2]).then(([result1, result2]) => {
    ...
    return (
        <div className="App">
          ...
          <Route path="/detail/2" element={<Detail shoes={shoes}/>}/> */}
            ...
            {count > 1 ? null : <button onClick={loadShoesByOnce}>more</button>}
        </div>
    );
}
